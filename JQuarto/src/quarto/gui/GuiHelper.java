...
public class GuiHelper {
	...
	private static ImageIcon getTileIcon(String iconPath) {
		...
		Image tileImage = tileBufferedImage.getScaledInstance(TilesPanel.TILE_SIZE, TilesPanel.TILE_SIZE,
				Image.SCALE_SMOOTH);
	...
	private static Map<String, ImageIcon> getPiecesIcons(String input) {
		...
		for (String pieceNumber : Board.PIECES_NUMBERS_STRINGS) {
			...
			if (input.equals("Slot")) {
				pieceImage = pieceBufferedImage.getScaledInstance(TilesPanel.TILE_SIZE, TilesPanel.TILE_SIZE,
						Image.SCALE_SMOOTH);
			} else {
				pieceImage = pieceBufferedImage.getScaledInstance(PiecesPanel.PIECE_SIZE, PiecesPanel.PIECE_SIZE,
						Image.SCALE_SMOOTH);
			}
