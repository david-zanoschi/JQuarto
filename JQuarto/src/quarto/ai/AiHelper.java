...
public class AiHelper {
	public static boolean isAiMove(GameWindow gameWindow) {
		// returns true if opponent is ai and ai moves first and it's odd move or ai
		// moves second and it's even move
		return gameWindow.isAiOpponentSelected() && ((StateManager.isAiMovingFirst && StateManager.moveCounter % 2 == 1)
	...
	// iterate through all remaining pieces, pick the non-winning ones and store
	// them
	public static List<Piece> getNonWinningPieces(Board boardParam, Piece chosenPiece) {
		...
		for (Piece remainingPiece : remainingPieces) {
			if (remainingPiece == null) {
				...
				// the chosen piece is still a remaining piece, so it must be filtered out when
				// considering what piece to choose for opponent
			...
			// place the piece on all empty tiles; if no placement wins the game, store the
			// piece
	...
	// stringBuilder.append(piecePosition < 10 ? "0" + piecePosition :
	// piecePosition);
	// // separator
	// stringBuilder.append(",");
	// if (board.isGameOver())
