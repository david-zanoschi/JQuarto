...
public class AiPlayer {
    // place piece on winning position; if no winning position available, place the
    // piece so the opponent won't invariably win with next move
    public static void placePiece(Board board) {
        ...
        for (Integer coordinate : emptyTilesCoordinates) {
            ...
            // if there are non-winning pieces for the current coordinate, store the
            // coordinate
    ...
    public static void choosePiece(PiecesPanel piecesPanel) {
        ...
        if (nonWinningPieces.size() > 0) {
            // the reason for this piece of code is that remaining pieces and noWinning
            // pieces are different objects
            // so choosing a piece from nonWinningPieces itself will not actually choose a
            // piece from the game board
            Piece chosenPiece;
